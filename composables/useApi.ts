import { useI18n } from 'vue-i18n'
export const useApi = (url) => {
    const { locale } = useI18n()
    // let authToken = useCookie('auth_token') // 假设您将 token 存储在 cookie 中
    let authToken = ""
    // 统一处理 locale 格式
    let urlWithLocale = url;
    // const currentLocale = locale.value === 'zh' ? 'zh-Hans' : locale.value;
    
    // if (currentLocale !== 'en') {
    //     // 假设您的 API 使用查询参数 'locale'
    //     const separator = urlWithLocale.includes('?') ? '&' : '?';
    //     urlWithLocale += `${separator}locale=${currentLocale}`;
    // }
    
    // const dynamicKey = `api-${currentLocale}-${url}`;
    return useFetch(urlWithLocale, {
      baseURL: 'https://cms.stoneboss.vip/api', // 统一设置基础 URL
      timeout: 50000,
      server: true,
      retry: 2,
      retryDelay: 500,
      headers: {
        Authorization: `Bearer ${authToken}`,
      },
       onResponseError(error) { 
          console.log('error', error)
        /* 记录错误，避免抛异常 */ },
    });
  };